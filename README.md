...
## Part 10 — Course capacity & enrollment
- Added `capacity` and `enrolledStudents` to `Course`.
- Implemented `RegistrationService.enrollStudentInCourse(...)`.
- Updated persistence format (`courses.txt` now stores capacity).
- Verified enrollment fails when capacity exceeded.

### SOLID reflection
- Single Responsibility: Course handles state; RegistrationService handles enrollment logic.
- Dependency Injection: services can be injected into `Main` for easier testing.
...
